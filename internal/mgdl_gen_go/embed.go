...
func genType(image *idl.Image, t *proto.TypeSpecifier) string {
	...
	default:
		// TODO 2024.01.02: data, virtual(list, presence), enum, annotation
	...
	for _, target := range targets {
		...
		for _, module := range image.Modules {
			if module.URI == targetURI {
				...
				// TODO 2024.01.03: support path;package syntax
				...
				packageName := GoSanitized(packageValue.Kind.(*proto.Value_Text).Text.Value)
				...
				for _, sdk := range module.SDKs {
					ifName := fmt.Sprintf("%sServer", sdk.Name.Name)
					g.P("// type ", ifName, " is the server API for ", sdk.Name.Name, "SDK.")
