		...
		for _, module := range gen.image.Modules {
			if module.URI == targetURI {
				...
				for _, constant := range module.Constants {
					...
					g.P("const ", constant.Name, " ", gen.genType(gen.image, constant.Type), " = ", genLiteral(constant.Value))
					g.P()
				}
				...
				if gen.opts.renderAPIs {
					...
					for _, api := range module.APIs {
						...
						for _, ext := range api.Extends {
							g.P("    ", gen.genType(gen.image, ext))
						}
						for _, method := range api.Methods {
							g.P("    ", method.Name, "(ctx context.Context, req ", gen.genType(gen.image, method.Input), ") (", gen.genType(gen.image, method.Output), ", error)")
						}
						g.P("}")
						g.P()
					}
				...
				for _, sdk := range module.SDKs {
					...
					for _, method := range sdk.Methods {
						...
						for _, input := range method.Input {
							...
							arguments += gen.genType(gen.image, input.Type)
						}
						rtype := ""
						if method.Output != nil {
							rtype = gen.genType(gen.image, method.Output)
						}
						if !method.NoThrows {
							if rtype == "" {
								rtype = "error"
...
func (gen *Generator) genType(image *idl.Image, t *proto.TypeSpecifier) string {
	...
	case idl.TypeKindAPI:
		if gen.opts.renderAPIs {
			...
			return declaration.(*proto.API).Name.Name
		}
	...
	case idl.TypeKindVirtual:
		switch declaration.(*proto.Struct).Name.Name {
		case "List":
			return fmt.Sprintf("[]%s", gen.genType(image, resolved.Parameters[0]))
		case "Presence":
			return fmt.Sprintf("*%s", gen.genType(image, resolved.Parameters[0]))
		case "Map":
			return fmt.Sprintf("map[%s]%s", gen.genType(image, resolved.Parameters[0]), gen.genType(image, resolved.Parameters[1]))
		default:
			panic("unsupported virtual type in mgdl-gen-go")
		}
	case idl.TypeKindEnum:
		return declaration.(*proto.Enum).Name
	default:
