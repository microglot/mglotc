...
func (self *compiler) Compile(ctx context.Context, req *idl.CompileRequest) (*idl.CompileResponse, error) {
	...
	for _, file := range files {
		go func(file idl.File) {
			image, err := self.compileFile(ctx, file, loaded, req.DumpTokens)
...
func (self *compiler) compileFile(ctx context.Context, file idl.File, loaded *sync.Map, dumpTokens bool) (*idl.Module, error) {
	self.Semaphore.Lock()
	...
	return sc.CompileFile(ctx, self.Reporter, file, dumpTokens)
}
