...
func TestUnicodeFileBody(t *testing.T) {
	...
	f := fs.NewFileString("/test", unicodeText, idl.FileKindNone)
	b, err := f.Body(ctx)
	require.NoError(t, err)
	it := NewUnicodeFileBody(b)
	expected := utf8.RuneCountInString(unicodeText)
	found := 0
...
var benchUnicodeEscape idl.CodePoint

func BenchmarkUnicodeFileBody(b *testing.B) {
	ctx := context.Background()
	f := fs.NewFileString("/test", unicodeText, idl.FileKindNone)
	body, err := f.Body(ctx)
	require.NoError(b, err)
	it := NewUnicodeFileBody(body)
	b.ResetTimer()
	for n := 0; n < b.N; n = n + 1 {
		benchUnicodeEscape = it.Next(ctx).Value()
	}
}

const unicodeText = `Sentences that contain all letters commonly used in a language
--------------------------------------------------------------
